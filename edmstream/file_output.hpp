...
double update_time_total_local_density = 0;
double update_time_total_dependent_point = 0;
...
// result output
void output_result(const unsigned int counter, const bool flag, const unsigned int seed_set_size) {
    ...
    if (flag == 1) file << (local_density_update_time + dependent_point_update_time) / (freq * 1000) << "," << memory << "," << local_density_update_time / (freq * 1000) << "," << dependent_point_update_time / (freq * 1000) << "," << scan_num << "," << subscan_num << "," << seed_set_size << "\n";
    if (flag == 0) file << update_time_total / (1000 * counter) << "," << memory << "," << update_time_total_local_density / (1000 * counter) << "," << update_time_total_dependent_point / (1000 * counter) << "," << counter << "\n";
...
// output local density & dependent point
void output_statistics(const unsigned int counter, std::deque<data*> &dataset_active) {
    ...
    for (unsigned int i = 0; i < dataset_active.size(); ++i) file << dataset_active[i]->identifier << "," << dataset_active[i]->local_density << "," << dataset_active[i]->dependent_point_id << "\n";
}