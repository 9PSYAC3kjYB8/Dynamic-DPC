...
int main() {
    ...
    // a set of seed objects (not deleted)
    std::deque<data*> seed_set;
    ...
    while (1) {
        ...
        if (flag) {
            ...
            std::vector<unsigned int> seedset_rho_update;   // element is an idx of dataset_active
            update_local_density_insertion(seedset_rho_update, seed_set, dataset_active, &dataset[idx_insertion]);
            ...
            update_dependent_point_insertion(seedset_rho_update, dataset_active, &dataset[idx_insertion]);
            ...
            // delete the front seed if dat is seed
            if (dat->seed_flag) seed_set.pop_front();

            // local density computation
            update_local_density_deletion(seed_set, dat);
        ...
        if (idx_insertion == dataset.size() - 1) {
            ...
            // output final result
            output_result(counter, 0, seed_set.size());
        ...
        if (counter % freq == 0) {
            ...
            output_result(counter, 1, seed_set.size());
        }
